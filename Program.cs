...
{
    var servers = await db.Servers.OrderBy(x => x.Id).Select(x => new {
        x.Id, x.Name, x.Description,
        PlayerCount = members.TryGetValue(x.Id, out var m) ? m.Count : 0
    }).ToListAsync();
...
app.MapPost("/api/login", async (LoginRequest req, AppDb db) =>
{
    ...
    return Results.Ok(new { token, username = user.Username, isAdmin = user.IsAdmin });
});

app.MapPost("/api/logout", (AuthRequest req) =>
{
    if (!string.IsNullOrWhiteSpace(req.Token)) sessions.TryRemove(req.Token, out _);
    return Results.Ok();
});

app.MapPost("/api/servers/{id:int}/join", async (int id, AuthRequest req, AppDb db) =>
{
    if (!TryGetUser(req.Token, out var username)) return Results.Unauthorized();
...
{
    if (!TryGetUser(token, out _)) return Results.Unauthorized();
    var messages = await db.Messages.Where(x => x.ServerId == id)
        .OrderByDescending(x => x.Id).Take(100).OrderBy(x => x.Id)
        .Select(x => new { x.Id, x.Username, x.Text, x.CreatedAt }).ToListAsync();
...
app.MapPost("/api/servers/{id:int}/messages", async (int id, SendMessageRequest req, AppDb db) =>
{
    ...
    var message = new ChatMessage {
        ServerId = id, Username = username!, Text = req.Text.Trim(), CreatedAt = DateTime.UtcNow
    };
...
{
    if (!TryGetUser(token, out var username)) return Results.Unauthorized();
    var user = await db.Users.FirstOrDefaultAsync(x => x.Username == username);
    return user == null ? Results.Unauthorized() : Results.Ok(new { user.Username, user.IsAdmin });
});

app.MapPost("/api/admin/kick", async (AdminActionRequest req, AppDb db) =>
{
    if (!await IsAdmin(req.Token, db)) return Results.Forbid();
...
app.MapPost("/api/admin/ban", async (BanRequest req, AppDb db) =>
{
    ...
    {
        db.Bans.Add(new Ban { ServerId = req.ServerId, Username = req.Username });
        await db.SaveChangesAsync();
    }
    if (members.TryGetValue(req.ServerId, out var serverMembers))
        serverMembers.TryRemove(req.Username, out _);
...
{
    username = null;
    return !string.IsNullOrWhiteSpace(token) && sessions.TryGetValue(token, out username);
}

async Task<bool> IsAdmin(string? token, AppDb db)
{
    if (!TryGetUser(token, out var username)) return false;
...
public class User { public int Id { get; set; } public string Username { get; set; } = ""; public string PasswordHash { get; set; } = ""; public bool IsAdmin { get; set; } }
public class Server { public int Id { get; set; } public string Name { get; set; } = ""; public string Description { get; set; } = ""; }
public class ChatMessage { public int Id { get; set; } public int ServerId { get; set; } public string Username { get; set; } = ""; public string Text { get; set; } = ""; public DateTime CreatedAt { get; set; } }
public class Ban { public int Id { get; set; } public int ServerId { get; set; } public string Username { get; set; } = ""; }
